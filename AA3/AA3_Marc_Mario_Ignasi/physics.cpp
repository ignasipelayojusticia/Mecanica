...
public:
	...
	void InitStraw(float strawHeight)
	{
		...
		for (int i = 1; i < Fiber::numVerts; i++)
		{
			...
			springs.emplace_back(k_stretch, ((i - 1) * 3), i * 3, 1);

			if (bend)
				springs.emplace_back(k_bend, ((i - 2) * 3), i * 3, 2);
			else
				bend = true;
			y++;
		}
...
glm::vec3 computeForces(FiberStraw& fiber, int idx, const std::vector<ForceActuator*>& force_acts)
{
	...
	for (auto force_act : force_acts)
		force += force_act->computeForce(1, { fiber.pos[idx * 3], fiber.pos[idx * 3 + 1], fiber.pos[idx * 3 + 2] });
...
void verlet(float dt, FiberStraw& fiber, const std::vector<Collider*>& colliders, const std::vector<ForceActuator*>& force_acts)
{
	...
	for (int i = 1; i < Fiber::numVerts; i++)
	{
		...
		for (auto collider : colliders)
			if (collider->checkCollision(pos, nextPos)) {
				...
				collider->computeCollision(pos, newPos, newVel);
...
void renderPrims() {
	...
	if (renderParticles)
	{
		const int startDrawingFromParticle = 0;
		const int numParticlesToDraw = Particles::maxParticles;
		Particles::drawParticles(startDrawingFromParticle, numParticlesToDraw);
	}
	if (renderMesh)
		Mesh::drawMesh();
	if (renderFiber)
		for (auto& fiberStraw : fiberStraws)
			fiberStraw.DrawFiberStraw();
	if (renderCube)
		Cube::drawCube();
}
...
{
	for (auto& fiberStraw : fiberStraws)
		fiberStraw.ResetStraw(strawHeight);
}
...
void GUI() {
	...
	ImGui::Begin("Physics Parameters", &show, 0);
	{
		...
		if (ImGui::TreeNode("Forces"))
		{
			...
			ImGui::DragFloat3("Wind Acceleration", &wind->force.x, dragPrecision);
			ImGui::TreePop();
		}
	}
	ImGui::End();
...
void PhysicsInit()
{
	...
	for (int i = 0; i < numberOfStraws; i++)
		fiberStraws.emplace_back(strawHeight);
...
void PhysicsUpdate(const float dt)
{
	...
	for (int j = 0; j < 20; j++)
		for (auto& fiber_straw : fiberStraws)
			verlet(dt / 20, fiber_straw, colliders, forceActuators);
}
